...
function ckgwwc_CloudKassir()
{
	...
	class WC_CloudKassir extends WC_Payment_Gateway
	{
		...
		public function init_form_fields()
		{
			...
			foreach ($gateways as $paym => $k){$this->form_fields['payment_methods']['options'][$paym] = __($k, 'woocommerce');}
		}
		...
		public function ckgwwc_Object_to_array($data)
		{
			if (is_array($data) || is_object($data)) {
				$result = array();
				foreach ($data as $key => $value){$result[$key] = self::ckgwwc_Object_to_array($value);}
				return $result;
			}
			return $data;
		}
		...
		public function ckgwwc_SendReceipt($order, $type, $old_status, $new_status)
		{
			...
			foreach ($cart as $item_id => $item_data):
				...
				if ("wc-" . $new_status == $this->status_delivered) $method = 4;
                else $method = (int)$this->kassa_method;

				//вычисление скидки
				$fees = $order->get_items('fee');
				foreach ($fees as $fee) { $total_fees = floatval($fee->get_total());}
			...
			if ($order->get_total_shipping()):
				...
				if ($this->shipping_discount) $total_amount = $total_amount + number_format(floatval($order->get_total_shipping()), 2, ".", '');
		...
		{
			$request2 = self::ckgwwc_curl_json_encode($request);
			...
			if (!empty($body->Message)) self::ckgwwc_addError('Check create resp message: ' . mb_convert_encoding($body->Message, "windows-1251", "utf-8"));
		...
		public function ckgwwc_update_order_status($order_id, $old_status, $new_status) //OK
		{
			...
			if (!empty($this->payment_methods) && in_array($order->get_payment_method(), $this->payment_methods)):
				if ($this->status_pay == "wc-" . $new_status || ("wc-" . $new_status == $this->status_delivered && in_array((int)$this->kassa_method, array(1, 2, 3)))):
		...
		function ckgwwc_curl_json_encode($a = false)      /////ok
		{
			if (is_null($a) || is_resource($a)) return 'null';
			...
			if ($isList) {
				foreach ($a as $v) { $result[] = self::ckgwwc_curl_json_encode($v);}

				return '[ ' . join(', ', $result) . ' ]';
			} else {
				foreach ($a as $k => $v) {$result[] = self::ckgwwc_curl_json_encode($k) . ': ' . self::ckgwwc_curl_json_encode($v);}
		...
		public function ckgwwc_callback()
		{
			if ($_GET['action'] == 'receipt') {
				...
				if ($_POST['Type'] == 'IncomeReturn') $Type = 'возврата прихода';
                elseif ($_POST['Type'] == 'Income') $Type = 'прихода';
				$url = $_POST['Url'];
